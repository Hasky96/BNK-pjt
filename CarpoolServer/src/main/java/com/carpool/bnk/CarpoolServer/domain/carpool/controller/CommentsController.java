...
import com.carpool.bnk.CarpoolServer.domain.carpool.response.AllCommentRes;
...
public class CommentsController {
    ...
    @GetMapping("/{carpoolNo}/comments")
    public ResponseEntity<?> getComments(@PathVariable("carpoolNo")int carpoolNo){
        Carpool carpool = carpoolRepository.getCarpoolByCarpoolNo(carpoolNo);
        List<Comments> list = carpool.getComments();
        return ResponseEntity.status(200).body(new AllCommentRes(list, carpoolNo));
    }
}
